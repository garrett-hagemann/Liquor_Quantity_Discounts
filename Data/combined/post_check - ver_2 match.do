...
import delimited using "ver_2/upc_crosswalk_checked.csv"
...
/*
gen product = substr(string(upc_id,"%12.0f"),6,.) if purchase_month < 10
replace product = substr(string(upc_id,"%12.0f"),7,.) if purchase_month >= 10
...
preserve
	use upc_file, clear
	split brand_string, gen(tmp) parse(";")
	replace brand_string = tmp1 + ";" + tmp2 + ";" + tmp3 + ";" + tmp4
	tempfile new_upc_file
	save `new_upc_file'
restore

merge 1:m purchase_month purchase_year brand_string using `new_upc_file', keep(2 3) gen(_merge_matched)
drop if product == 0

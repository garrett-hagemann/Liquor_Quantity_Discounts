  ...
  for hs_n = 2:N
    if hs_n == 2 # special case for N=2 which is a scalar problem in constrained version
      ...
      hs_x0 = [hs_rhos ; hs_types] #
        ...
        =#
        ps_optim_res = Optim.optimize((x)->g2(x,hs_n),(g,x)->focs!(g,x,hs_n),(m,x)->hess!(m,x,hs_n),hs_x0,method=NewtonTrustRegion(), g_tol=1e-4, iterations=10000, show_trace=false, extended_trace=false,allow_f_increases=false)

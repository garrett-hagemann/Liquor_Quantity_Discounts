...
test_coefs = DemandCoefs(-0.7,0.0,0.0,0.02,0.0,0.0,Dict("gin"=>.7,"vod"=>.7))
test_inc = IncomeDist([30000.0,60000.0],[.75,.25])
...
test_Q = moment_obj_func(test_ps,test_devs,test_w_params,test_prod1,test_coefs,test_inc,test_mkt,test_pre_calc)
println("Q(true params) = ", test_Q) # should b 0
test_Q2 = moment_obj_func(test_ps,test_devs,test_w_params2,test_prod1,test_coefs,test_inc,test_mkt,test_pre_calc)
println("Q(other params) = ", test_Q2) # should be > 0

# testing moment optimization to recover params
optimize_moment(test_ps,test_devs,test_prod1,test_coefs,test_inc,test_mkt,1,test_pre_calc,x0=[0.0,1.0,1.0])
@time test_recovered_params,test_xtrace,test_ftrace = optimize_moment(test_ps,test_devs,test_prod1,test_coefs,test_inc,test_mkt,25000,test_pre_calc,x0=[0.0,1.0,1.0])
